...
void main(void)
{
	float size = min(u_screenSize.x, u_screenSize.y);
    float pixSize = 1.0 / size;
    vec2 fragCoord = vec2(texCoord.x * u_screenSize.x, (1.0 - texCoord.y) * u_screenSize.y);
	vec2 uv = fragCoord.xy / u_screenSize.x;
    float stroke = pixSize * 1.5;
    vec2 center = vec2(0.5, 0.5 * u_screenSize.y/u_screenSize.x);
